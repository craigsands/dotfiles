...
# Line 2: launched · context · spend · tokens · [tasks]
#
# Context bar colors:
#   Green  = < 70% used (nominal)
#   Yellow = 70–89% (approaching limit)
#   Red    = 90%+ (critical)
#
# Context display:
#   ███░░░░░░░ 30%  — real data from API or transcript
#   ···             — no data yet (fresh session, pre-first-message)
#
# Model display colors:
#   Default  = Sonnet / Haiku (standard cost)
#   Magenta  = Opus (expensive)
#   Red bold = Max mode (expensive)
#   + [MAX] / [THINK] badges when those modes are active
#
# REQUIREMENTS
# ------------
...
if [ "$PCT_RAW" = "null" ] && [ -n "$TRANSCRIPT" ] && [ -f "$TRANSCRIPT" ]; then
    # Use last assistant message's total tokens — each API call sends full context
    ACTUAL_TOKENS=$(jq -rs '
...
if [ -n "$TRANSCRIPT" ] && [ -f "$TRANSCRIPT" ]; then
    ACTIVE_TASKS=$(jq -rs '
      ...
      [.[] | (.message.content? // []) | .[]? |
        select(.type == "tool_result") | .tool_use_id] as $resolved |
      [$task_ids[] | select(. as $id | $resolved | any(. == $id) | not)] | length
    ' "$TRANSCRIPT" 2>/dev/null)
fi
